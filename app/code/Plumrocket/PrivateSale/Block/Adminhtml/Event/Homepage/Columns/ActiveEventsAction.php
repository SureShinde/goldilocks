...
class ActiveEventsAction extends AbstractColumn
{
    /**
     * @inheritDoc
     */
    protected function initEventsData(int $categoryId): AbstractColumn
    {
        ...
        $count = $collection->addActiveFilters()->getSize();

        $this->setLabel((string) $count)
             ->setUrlParams(['status' => implode('-', [EventStatus::ACTIVE, EventStatus::ENDING_SOON])])
