...
class Inventory extends AbstractDb
{
    ...
    public function getMsiQty($productSku, $websiteCode)
    {
        if (!isset($this->qty[$websiteCode][$productSku])) {
            $itemQty = $this->getItemQty($productSku, $websiteCode);
            if ($itemQty !== false) {
                $this->qty[$websiteCode][$productSku] = $itemQty
                    + $this->getReservationQty($productSku, $this->getStockId($websiteCode));
            } else {
                // mean that product not assigned for sources of current website
                $this->qty[$websiteCode][$productSku] = null;
            }
        }
    ...
    public function clearQtyCache(): void
    {
        $this->qty = [];
    }
}
