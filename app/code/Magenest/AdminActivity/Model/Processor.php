...
class Processor
{
	...
	public function __construct(
		\Magenest\AdminActivity\Model\Config $config,
		\Magento\Backend\Model\Auth\Session $authSession,
		\Magenest\AdminActivity\Model\Handler $handler,
		\Magento\Framework\HTTP\PhpEnvironment\RemoteAddress $remoteAddress,
		\Magenest\AdminActivity\Model\ActivityFactory $activityFactory,
		\Magenest\AdminActivity\Model\ActivityLogDetailFactory $activityDetailFactory,
		...
		\Magenest\AdminActivity\Model\Activity\Status $status,
		\Magenest\AdminActivity\Model\Handler\PostDispatch $postDispatch,
	...
	{
		if (\Magenest\AdminActivity\Helper\Data::isWildCardModel($model)) {
	...
	public function addLog($activity, $logData, $model)
	{
		...
		$this->activityLogs[] = [
			\Magenest\AdminActivity\Model\Activity::class          => $activity,
			\Magenest\AdminActivity\Model\ActivityLog::class       => $logData,
			\Magenest\AdminActivity\Model\ActivityLogDetail::class => $logDetail
		];
	}
		...
		try {
			if (!empty($this->activityLogs)) {
				foreach ($this->activityLogs as $model) {
					$activity = $model[\Magenest\AdminActivity\Model\Activity::class];
					$activity->save();
					$activityId = $activity->getId();

					if (isset($model[\Magenest\AdminActivity\Model\ActivityLog::class])) {
						$logData = $model[\Magenest\AdminActivity\Model\ActivityLog::class];
						if ($logData) {
					...
					if (isset($model[\Magenest\AdminActivity\Model\ActivityLogDetail::class])) {
						$detail = $model[\Magenest\AdminActivity\Model\ActivityLogDetail::class];
	...
	public function _initLog()
	{
		...
		if ($this->authSession->isLoggedIn()) {
            ...
            $activity->setRequest($this->json->serialize($this->httpRequest->getParams()));
		} else {
            $activity->setRequest('{}');
        }
